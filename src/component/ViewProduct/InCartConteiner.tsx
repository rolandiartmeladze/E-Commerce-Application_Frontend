...
const InCartConteiner = ({
  ...
  const fetchData = async () => {
    try {
      ...
      const checkCartItem = await fetch(
        `https://lavish-husky-gaura.glitch.me/checkCartItems`,
        {
          method: 'POST',
      ...
      if (quantities.length <= 0) {
        const initialQuantities = cartResponse.map((item: CartItem) => ({
          id: item._id,
          quantity: 1,
        }));
  ...
  const BuyBtn = () => {
    const handleAppendToContainer = (): Node => {
      ...
      const buyFromCartString =
        ReactDOMServer.renderToString(buyFromCartElement);
  ...
  const callproductfromcart = (
    e: React.MouseEvent<HTMLAnchorElement, MouseEvent>,
    { productID, setProduct, setLoading, index }: callprops
  ) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  return (
    <>
          ...
          <>
            ...
            >
              <ProductConteiner id={`product${index}`} key={producti._id}>
                ...
                <ImgConteiner>
                  <img
                    src={`${Media}/${producti.userID}/${producti.image[0]}`}
                    alt="img"
                  />
                </ImgConteiner>
                <ProductInfoCont>
                  <samp>
                    Name: {producti.name.substring(0, 16)}
                    {producti.owner.length > 16 && '...'}
                  </samp>
                  <samp>
                    Price: {producti.price.toFixed(1)} {producti.currency}
                  </samp>

                  <samp>
                    Cost:{' '}
                    {(producti.price * quantities[index].quantity).toFixed(1)}{' '}
                    {producti.currency}
                  </samp>
                </ProductInfoCont>
                <div
                ...
                >
                  ...
                  <ProductQuantityCont id={producti._id}>
                    <button onClick={() => removecart(producti._id, index)}>
                      {' '}
                      Delete
                    </button>
                  </ProductQuantityCont>
      ...
      >
        <BuyBtnII disabled={incartResponse.length <= 0} onClick={BuyBtn}>
          Buy Now
        </BuyBtnII>
      </div>
    </>
