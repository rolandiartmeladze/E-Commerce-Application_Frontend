...
const Home = ({ setProduct }: Props) => {
  ...
  const ActuallyCategory = respons.filter(
    (product: any) => product.category === category
  );

  useEffect(() => {
    fetchData(setLoading, setRespons);
  }, []);
  ...
  const MoreButton: React.FC<BtnProps> = ({ link }) => {
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <MoreBtn onClick={handleClick}>Show More...</MoreBtn>
      </div>
    );
  };
  ...
  return (
    <>
      <InfoBoard>
        <ul>
          <li>
            {<img src={Product} alt="" />}Products: {respons.length}
          </li>
          <li>
            {<img src={Person} alt="" />}Members: {memberLength}
          </li>
          <li>
            {<img src={Category} alt="" />}Categorys: {'5'}
          </li>

          <li>
            {<img src={View} alt="" />}View: {TotalView}
          </li>
          <li>
            {<img src={Guset} alt="" />}Guests: {'0'}
          </li>
          <li>
            {<img src={Sale} alt="" />}Sales: {TotalSale}
          </li>
        </ul>
      </InfoBoard>
      ...
      <Conteiner className="cont">
        ...
        {!loading && (
          <ProductComponent
            products={Actually.slice(0, item)}
            {...ProductProps}
          />
        )}
      ...
      <Conteiner className="cont">
        ...
        {!loading && (
          <ProductComponent
            products={ActuallyCategory.slice(0, item)}
            {...ProductProps}
          />
        )}
