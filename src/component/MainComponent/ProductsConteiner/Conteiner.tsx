...
const Product = styled.article`
  ...
  border-radius: 8px;
  border: 2px rgb(253, 253, 253, 0) solid;
  position: relative;
...
const Container: React.FC<{ products: Props[]; setSelected: Function }> = ({
  products,
  setSelected,
}) => {
      ...
      <ConteinerComponent>
        ...
        {products.map((item, index) => (
          ...
          >
            ...
            <ProductHead>
              <img
                src={`${Media}/${item.userID}/${item.image[0]}`}
                alt="User Icon"
              />
            </ProductHead>
            ...
            <Info>
              <ul>
                ...
                <div>
                  <b>price:</b>
                  <samp>
                    {item.price.toFixed(1)} {item.currency}
                  </samp>
                </div>
                <div>
                  <b>cost:</b>
                  <samp>
                    {(item.quantity * item.price).toFixed(1)} {item.currency}
                  </samp>
                </div>
              </ul>
              ...
              >
                <div>
                  {item.view}
                  <img width={15} src={vewicon} alt="view" />
                </div>
                <div>
                  {item.sale}
                  <img width={15} src={saleicon} alt="sale" />
                </div>
                <div>
                  {item.share}
                  <img width={15} src={shareicon} alt="share" />
                </div>
              </ul>
            </Info>
          </Product>
