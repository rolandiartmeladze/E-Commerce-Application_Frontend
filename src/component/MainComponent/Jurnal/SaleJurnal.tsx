...
const SaleJurnal: React.FC = () => {
  ...
  async function fetchData(sort: string): Promise<void> {
    ...
    try {
      const response = await fetch(
        `https://quasar-wind-trader.glitch.me/api/salejurnal/${token}/?sort=${sort}`,
        {
          method: 'GET',
  ...
  useEffect(() => {
    if (resspons && resspons.length > 0) {
      ...
      const totalCost = resspons.reduce(
  ...
  const groupRessponsByYearMonthDay = (
    resspons: Product[]
  ): GroupedProducts => {
  ...
  const total = (result: any) => {
    ...
    return (
      <div className="result">
        <samp>
          {' '}
          <img src={productIcon} alt="" /> {saleProduct}
        </samp>
        <samp>
          {' '}
          <img src={costIcon} alt="" /> {totalCost.toFixed(2)}
        </samp>
      </div>
    );
  };

  const sortKeysDesc = (obj: { [key: string]: any }) => {
    return Object.keys(obj).sort((a, b) => Number(b) - Number(a));
  };
            ...
            <section key={year} className="year-section">
              ...
              >
                &nbsp; {year}
                {
                  <img
                    style={
                      hiddenYear[keyYear] || year === thisYear
                        ? iconstyle
                        : undefined
                    }
                    src={Arrow}
                    ...
                    <div key={month} className="month-div">
                      ...
                      >
                        &ensp; {monthis(month)}
                        {
                          <img
                            style={
                              hiddenMonth[keyMonth] || month === thisMonth
                                ? iconstyle
                                : undefined
                            }
                            src={Arrow}
                      ...
                      {(hiddenMonth[keyMonth] || month === thisMonth) &&
                        sortKeysDesc(groupedResspons[year][month]).map(
                          (day) => {
                            const key = `${year}-${month}-${day}`;
                                  ...
                                  products.map((product, index) => (
                                    <Conteiner key={index}>
                                      ...
                                      <Info>
                                        ...
                                        <samp>
                                          <b>Time:</b> {product.formattedTime}
                                        </samp>
                                      </Info>
                                    </Conteiner>
                                  ))}
