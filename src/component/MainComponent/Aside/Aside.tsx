...
const Aside = ({ product, setMyProducts, setProduct, setLoading }: Props) => {
  ...
  const minus = () => {
    product && setSoldAmount(Math.max(0, soldAmount - 1));
  };

  const plus = () => {
    if (product && soldAmount < product.quantity) {
  ...
  const SaleNow = async () => {
    ...
    setLoading(true);
    try {
      const saleProduct = await fetch(
        `https://quasar-wind-trader.glitch.me/api/sale/${info.user}`,
        {
          method: 'POST',
    ...
    <AsideContainer>
        ...
        <>
          <AsideMessage>
            ...
            <MessageBody>
              ...
              <div>
                <b>quantity:</b> {soldAmount} {product.quantityUnit}{' '}
              </div>
              <div className="cost">
                <b>cost:</b> {(product.price * soldAmount).toFixed(2)}{' '}
                {product.currency}
              </div>
            </MessageBody>
            <button
      ...
      <ul>
        ...
        <Cost>
          <b>Total:</b>
          <samp>
            {(product?.price * soldAmount || 0).toFixed(2)} {product?.currency}
          </samp>
        </Cost>
      </ul>
