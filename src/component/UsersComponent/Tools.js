...
const Form = styled.form`
  ...
  .add {
    ...
    button {
      ...
      padding: 10px 20px;
      &:hover {
        box-shadow: 0.3px 0.3px 1px 0.5px rgb(0, 0, 0, 0.6);
      }
    }
  }
...
const FooterComp = () => {
  return (
    <Footer>
      <h3>
        If you have a problem or find a bug, please contact the administrator{' '}
        <samp>Contact</samp>
      </h3>
    </Footer>
  );
...
const checkPassword = (password) => {
  ...
  const isWeak =
    password === lower || password === upper || /^[a-zA-Z]+$/.test(password);
  ...
  if (password !== lower) {
    ...
    App.style.color = 'rgb(255, 0, 0 , 0.4)';
  }

  if (password !== upper) {
    Low.style.color = 'green';
  } else {
    Low.style.color = 'rgb(255, 0, 0 , 0.4)';
  }

  if (hasNumber) {
    Num.style.color = 'green';
  } else {
    Num.style.color = 'rgb(255, 0, 0 , 0.4)';
  }

  if (hasSpecialChar) {
    Sim.style.color = 'green';
  } else {
    Sim.style.color = 'rgb(255, 0, 0 , 0.4)';
  }

  if (hasMinLength) {
    Len.style.color = 'green';
  } else {
    Len.style.color = 'rgb(255, 0, 0 , 0.4)';
  }
...
const checkRepPassword = (reppassword, password) => {
  ...
  if (reppassword === password && reppassword.length > 0) {
    repPass.style.borderBottomColor = 'green';
  } else {
    repPass.style.borderBottomColor = 'red';
  }
};
